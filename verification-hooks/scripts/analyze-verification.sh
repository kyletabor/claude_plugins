...
VALID=$(wc -l < "$VALID_LOG")
INVALID=$((TOTAL - VALID))

echo "=== Verification Hooks Report ==="
echo "Log: $LOG ($VALID valid / $TOTAL total events${INVALID:+, $INVALID invalid})"
echo ""
...
' "$VALID_LOG" 2>/dev/null || echo "(no parseable events)"
echo ""
...
' "$VALID_LOG" 2>/dev/null || echo "(no timing data)"
echo ""

echo "--- Recent Blocks (last 5) ---"
jq -s '[ .[] | select(.action == "blocked") ] | .[-5:] | .[] | "\(.ts) [\(.gate)] \(.details.reason // "no reason")"' "$VALID_LOG" 2>/dev/null || echo "(none)"
echo ""

echo "--- Errors ---"
ERRORS=$(jq -s '[.[] | select(.action == "error")] | length' "$VALID_LOG" 2>/dev/null || echo 0)
if [ "$ERRORS" -gt 0 ]; then
  echo "WARNING: $ERRORS hook errors detected!"
  jq 'select(.action == "error") | "\(.ts) [\(.gate)] \(.details.error // "unknown")"' "$VALID_LOG" 2>/dev/null
else
  echo "No errors."
fi
echo ""
...
BD_TOTAL=$(jq -s '[.[] | select(.gate == "bd_close")] | length' "$VALID_LOG" 2>/dev/null || echo 0)
BD_BLOCKED=$(jq -s '[.[] | select(.gate == "bd_close" and .action == "blocked")] | length' "$VALID_LOG" 2>/dev/null || echo 0)
if [ "$BD_TOTAL" -gt 0 ]; then
  BD_RATE=$((BD_BLOCKED * 100 / BD_TOTAL))
  echo "| bd_close block rate | ${BD_RATE}% | 10-30% |"
fi

STOP_TOTAL=$(jq -s '[.[] | select(.gate == "stop")] | length' "$VALID_LOG" 2>/dev/null || echo 0)
STOP_BLOCKED=$(jq -s '[.[] | select(.gate == "stop" and .action == "blocked")] | length' "$VALID_LOG" 2>/dev/null || echo 0)
if [ "$STOP_TOTAL" -gt 0 ]; then
  STOP_RATE=$((STOP_BLOCKED * 100 / STOP_TOTAL))
  echo "| stop block rate | ${STOP_RATE}% | <10% |"
fi

ERROR_TOTAL=$(jq -s '[.[] | select(.action == "error")] | length' "$VALID_LOG" 2>/dev/null || echo 0)
